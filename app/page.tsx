...
export default async function Home() {
  ...
  // session.user is stripped (not the whole session) when the jwt callback
  // marked the token invalid, e.g. after a password change — check .user,
  // not the session object itself.
  if (!session?.user) {
    redirect("/login");
  }
