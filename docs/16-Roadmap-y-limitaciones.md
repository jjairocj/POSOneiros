...
- 🔄 **Retomado (2026-09-23):** cambió la topología — el servidor (CasaOS) quedó en una casa distinta a donde opera el POS, conectados por internet + cloudflared, no ya en la misma LAN/portátil. Cualquier corte de internet en cualquiera de las dos casas ahora tumba la conexión completa, no solo eventualmente. Se retoma el offline-first completo (catálogo cacheado, PWA shell) en el branch `feat/pos-offline-first` — ver [[18-Offline-First-POS]] para el diseño y roadmap detallado. La sesión JWT (`maxAge: 12h`, `app/api/auth/[...nextauth]/route.ts`) se revisó y es suficiente, no requiere cambios.
...
### Turno ligado al usuario, no a la caja (reportado 2026-09-25)
- **Síntoma:** con un CASHIER con turno abierto, si un ADMIN inicia sesión (mismo dispositivo u otro), el header del POS le muestra "Sin turno activo" en vez del turno en curso — el turno del cajero sigue abierto y válido, solo no es visible desde la sesión del admin.
- **Causa raíz:** `getActiveShift()` (`app/actions/shift.ts`) busca `where: { userId, status: {in: LIVE} }` — el turno es "de quien lo abrió", no "de la caja". El modelo `Shift` sí tiene `registerId` pero nunca se usa para esta consulta; no hay `@@unique` que ligue unicidad de turno abierto a la caja.
- **Fix aplicado (parche, no el rediseño completo):** el admin ahora ve un aviso "N turno(s) abierto(s)" (`OpenShiftsNotice.tsx`, usa `getOpenShifts()` ya existente) cuando no tiene turno propio, con lista de cajero/caja/hora de apertura y botón "Cerrar" por fila — reutiliza `ShiftClosingModal` tal cual, ya que `closeShift()`/`loadShiftForClose()` **ya permitían** que un SUPERVISOR/ADMIN cerrara el turno de otro usuario (ese candado ya existía, solo no había UI para llegar a él). Gateado por el permiso `VIEW_REPORTS` (delegable), no requiere permiso nuevo.
- **Pendiente (rediseño completo, descartado por ahora a pedido del usuario):** pasar el modelo a "turno por caja" — el dispositivo recordaría su `registerId` (localStorage, elegido una vez en `ShiftOpeningModal`) y `getActiveShift` buscaría por `registerId` en vez de `userId`. Así el badge diría "Turno activo: (dueño del turno)" para cualquiera que mire esa caja, y un cajero distinto no podría abrir turno ahí mientras esté ocupada (ese bloqueo también ya existe en `openShift`). Retomar si se vuelve a pedir explícitamente — toca `getActiveShift`, `ShiftHeader.tsx`, `ShiftClosingModal.tsx` (mostrar dueño al cerrar turno ajeno) y persistencia de `registerId` por dispositivo.
